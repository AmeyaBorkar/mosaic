...
fn dsl_density_facet_renders_in_sandbox_matching_native() {
    ...
    let out = sandbox
        .run_program(&facet, Limits::default(), &bytes, &features, n, 5)
        .unwrap();
...
fn dsl_edge_or_density_facet_sandbox_equals_reference() {
    // A real, branchy, spatial Facet: strong edges pick a directional glyph, else a
    // position-shaded density ramp (reads u/v) — so the sandbox==native proof covers the
    // position slots too.
    ...
    mosaic_vm::run(&validated, &features, n, 5, &mut native).unwrap();
